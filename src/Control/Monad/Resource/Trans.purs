...
newtype ResourceT (m :: Type -> Type) a = ResourceT (Registry -> m a)
...
runResourceT nat (ResourceT run) = do
  ...
  Aff.finally (Registry.cleanup registry) $
    nat (run registry)
...
joinResourceT (ResourceT run) =
  ResourceT \registry -> case run registry of ResourceT run' -> run' registry
...
instance Bind m => Bind (ResourceT m) where
  bind (ResourceT run) f =
    ResourceT \registry -> do
      a <- run registry
      case f a of ResourceT run' -> run' registry
...
instance MonadError e m => MonadError e (ResourceT m) where
  catchError (ResourceT run) h =
    ResourceT \registry -> catchError (run registry) \e -> case h e of ResourceT run' -> run' registry
...
instance (Monad m, Alt m) => Alt (ResourceT m) where
  alt (ResourceT run) (ResourceT run') = ResourceT \registry -> alt (run registry) (run' registry)

instance (Monad m, Plus m) => Plus (ResourceT m) where
  empty = lift empty

instance (Monad m, Alternative m) => Alternative (ResourceT m)

instance MonadPlus m => MonadPlus (ResourceT m)

instance Parallel f m => Parallel (ResourceT f) (ResourceT m) where
