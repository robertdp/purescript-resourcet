...
register runRelease =
  liftResourceT $ ResourceT \registry ->
    liftEffect $ Registry.register runRelease registry
...
deregister key =
  liftResourceT $ ResourceT \registry ->
    liftEffect $ Registry.deregister key registry
...
isRegistered key =
  liftResourceT $ ResourceT \registry ->
    liftEffect $ Registry.has key registry
...
fork child =
  liftResourceT $ ResourceT \registry ->
    Registry.forkAff (runResource child) registry

forkAff :: forall a m. MonadResource m => Aff a -> m (Fiber a)
forkAff aff =
  liftResourceT $ ResourceT \registry ->
    Registry.forkAff aff registry
